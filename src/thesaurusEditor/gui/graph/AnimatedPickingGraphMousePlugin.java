...
public class AnimatedPickingGraphMousePlugin<V, E> extends AbstractGraphMousePlugin
    ...
    public void animateToVertex(MouseEvent e, VisualizationViewer<V, E> vv) {
    	if (e.getModifiers() == modifiers) {
			GraphElementAccessor<V, E> pickSupport = vv.getPickSupport();
			PickedState<V> pickedVertexState = vv.getPickedVertexState();
            Layout<V,E> layout = vv.getGraphLayout();
    	...
    	if (e.getModifiers() == modifiers) {
            final VisualizationViewer<V,E> vv2 = vv;
            ...
            Runnable animator = new Runnable() {
                    ...
                    public void run() {
                            for (int i = 0; i < 10; i++) {
                                    vv2.getRenderContext().getMultiLayerTransformer().getTransformer(Layer.LAYOUT).translate(dx, dy);
                                    try {
                                            Thread.sleep(100);
